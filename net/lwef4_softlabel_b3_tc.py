...
class Encoder(nn.Module):
    def __init__(self, arch, pretrained=False):
        ...
        self.backbone = gen_efficientnet.__getattribute__(arch)(2, in_chans=4, pretrained=pretrained)
...
dummy_width = 512
dummy_x = torch.rand(1, 4, dummy_width, dummy_width)  # , device=encoder.device)
...
class LWef(nn.Module):
    ...
    def forward(self, x):
        ...
        # out_segm = F.max_pool2d(out_segm, 3, 1, 3//2)
        out_segm = (out_segm > 0.4).float()
        return out_segm
